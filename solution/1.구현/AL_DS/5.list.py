...
# [실생활 예시] : [알고리즘 본질]
# 1. 대기열 관리 : 순서가 있는 데이터 저장 및 관리
# 2. 재생 목록 : 음악/동영상 순서 있는 목록 관리
# 3. 최근 검색어 : 최근 항목부터 순서대로 저장

# [코딩테스트 꿀팁]
# 1. 슬라이싱 활용
#    - arr[1:3]: 1번부터 2번 인덱스까지
#    - arr[::-1]: 리스트 뒤집기
# 2. list comprehension
#    - [x for x in range(5)]: [0,1,2,3,4]
#    - [x for x in arr if x > 0]: 양수만 필터링
# 3. sort vs sorted
#    - sort(): 원본 리스트 정렬
...
# 실제 활용 예시 - 리스트 컴프리헨션과 필터링
numbers = [1, -2, 3, -4, 5, -6]
positive = [x for x in numbers if x > 0]  # 양수만 선택
doubled = [x * 2 for x in positive]      # 양수들을 2배로
print(positive)  # 출력: [1, 3, 5]
print(doubled)  # 출력: [2, 6, 10]
