...
def solution(N):
    ...
    def backtrack(curr_sum, selected, start):
        ...
        """
        등차수열(Arithmetic Sequence)은 연속된 두 항의 차이가 항상 일정한 수열을 말합니다.
        간단한 예시로 설명하면:
        [1, 3, 5, 7, 9] → 공차(차이)가 2인 등차수열
        [2, 4, 6, 8, 10] → 공차가 2인 등차수열
        3. [1, 2, 3, 4, 5] → 공차가 1인 등차수열
        우리 코드에서는 start부터 N까지의 연속된 자연수를 다루고 있습니다.
        예를 들어 start=3, N=5라면:
        수열: [3, 4, 5]
        공차: 1
        첫째항(a): 3
        ...
        합 = (첫째항 + 마지막항) × 항의 개수 ÷ 2
        # 실제로 3 + 4 + 5 = 12 입니다
        이렇게 등차수열의 합 공식을 사용하면 모든 숫자를 일일이 더하지 않고도 빠르게 합을 구할 수 있습니다.
        """
